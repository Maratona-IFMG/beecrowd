...
int mdc(int player_a, int player_b)
{
    if(player_a == 0) return player_b;
    return mdc( player_b % player_a, player_a);
}


int main()
{
    int N, player_a, player_b;

    scanf("%d", &N);
    for(int i = 0; i < N; i++)
    {
        scanf("%d %d", &player_a, &player_b);
        printf("%d\n", mdc(player_a, player_b));
    }
